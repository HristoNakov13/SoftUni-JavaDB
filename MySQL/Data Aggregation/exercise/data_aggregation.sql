...
SELECT SUBSTRING(`first_name`, 1, 1) AS `first_letter_name`
FROM `wizzard_deposits`
WHERE `deposit_group` = 'Troll Chest'
GROUP BY `first_letter_name`
ORDER BY `first_letter_name`;
...
SELECT COUNT(`salary`) AS `salaries_count_without_manager`
FROM `employees`
